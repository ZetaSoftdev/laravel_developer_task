            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="pt-3 edit-online-class-form" id="edit-form" action="{{ route('zoom.update', $onlineClass->id) }}" method="POST" novalidate="novalidate">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label for="title">{{ __('Title') }} <span class="text-danger">*</span></label>
                                    <input type="text" id="title" name="title" class="form-control" placeholder="{{ __('Enter Class Title') }}" value="{{ $onlineClass->title }}" required />
                                </div>
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="class_section_id" name="class_section_id" class="form-control select2" required>
                                        ...
                                        @foreach($classSections as $section)
                                            <option value="{{ $section->id }}" {{ $onlineClass->class_section_id == $section->id ? 'selected' : '' }}>{{ $section->class->name }} {{ $section->section->name }} {{ $section->medium->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="subject_id" name="subject_id" class="form-control select2" required>
                                        ...
                                        @foreach($subjects as $subject)
                                            <option value="{{ $subject->id }}" {{ $onlineClass->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="session_year_id" name="session_year_id" class="form-control select2" required>
                                        ...
                                        @foreach($sessionYears as $year)
                                            <option value="{{ $year->id }}" {{ $onlineClass->session_year_id == $year->id ? 'selected' : '' }}>{{ $year->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="datetime-local" id="start_time" name="start_time" class="form-control" value="{{ date('Y-m-d\TH:i', strtotime($onlineClass->start_time)) }}" required />
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="end_time">{{ __('End Time') }} <span class="text-danger">*</span></label>
                                    <input type="datetime-local" id="end_time" name="end_time" class="form-control" value="{{ date('Y-m-d\TH:i', strtotime($onlineClass->end_time)) }}" required />
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-12">
                                    <label for="description">{{ __('Description') }}</label>
                                    <textarea id="description" name="description" class="form-control" rows="4" placeholder="{{ __('Enter Class Description') }}">{{ $onlineClass->description }}</textarea>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-6">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" value="{{ ucfirst($onlineClass->status) }}" readonly />
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-md-12">
